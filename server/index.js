...
const server = http.createServer(async (request, response) => {
  try {
    ...
    const jobStatusMatch = url.pathname.match(/^\/api\/generation-jobs\/([a-zA-Z0-9_-]{8,64})$/);
    ...
    const eventsMatch = url.pathname.match(/^\/api\/generation-jobs\/([a-zA-Z0-9_-]{8,64})\/events$/);
    if (eventsMatch && request.method === 'GET') {
      return streamJobEvents(eventsMatch[1], request, response);
    }

    const enhanceMatch = url.pathname.match(/^\/api\/generation-jobs\/([a-zA-Z0-9_-]{8,64})\/enhance$/);
    if (enhanceMatch && request.method === 'POST') {
      return handleEnhance(enhanceMatch[1], request, response);
    }

    const previewTriggerMatch = url.pathname.match(/^\/api\/generation-jobs\/([a-zA-Z0-9_-]{8,64})\/preview$/);
    ...
    const deleteMatch = url.pathname.match(/^\/api\/views\/([a-zA-Z0-9_-]{8,64})$/);
    if (deleteMatch && request.method === 'DELETE') {
      return deleteView(deleteMatch[1], response);
    }

    const generated = url.pathname.match(/^\/gen\/([a-zA-Z0-9_-]{8,64})(?:\/(.*))?$/);
    if (generated && request.method === 'GET') {
      return serveGeneratedView(generated[1], generated[2] ?? '', response);
    }

    const previewMatch = url.pathname.match(/^\/gen-preview\/([a-zA-Z0-9_-]{8,64})(?:\/(.*))?$/);
...
function normaliseMainAppPath(urlPath) {
  if (urlPath === '/' || urlPath === '/docker') return '/index.html';
  if (urlPath.startsWith('/docker/')) {
    const withoutBase = urlPath.slice('/docker'.length);
