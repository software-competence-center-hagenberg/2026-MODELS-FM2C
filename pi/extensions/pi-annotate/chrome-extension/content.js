/**
 * Pi Annotate - Content Script (v0.4.1)
 * 
...
(() => {
  ...
  // Panel collapse state — when true, the panel shrinks to a header-only
  // strip so the bottom of the page (footers, sticky action bars) stays
  // reachable while the picker is active. Toggled via the collapse button
  // in the panel header or the "c" key. Persisted across reloads.
  ...
  function createPanel() {
    ...
    panelEl.id = "pi-panel";
    panelEl.classList.add("at-bottom");
    panelEl.innerHTML = `
      <div class="pi-header">
        ...
        <button class="pi-collapse" id="pi-collapse" title="Collapse panel (c)">▴</button>
      ...
      <div class="pi-toolbar">
        ...
        <button class="pi-mode-btn" id="pi-panel-toggle" title="Toggle panel position (top/bottom)" style="font-size:13px;line-height:1;">⇅</button>
      </div>
    ...
    // Collapse toggle
    const collapseBtn = document.getElementById("pi-collapse");
    collapseBtn.addEventListener("click", () => {
      panelCollapsed = !panelCollapsed;
      applyPanelCollapse();
    });
    // Apply persisted collapse state on activation
    applyPanelCollapse();
    
    // Panel position toggle
    document.getElementById("pi-panel-toggle").addEventListener("click", () => {
      if (panelEl.classList.contains("at-bottom")) {
        panelEl.classList.remove("at-bottom");
        panelEl.classList.add("at-top");
      } else {
        panelEl.classList.remove("at-top");
        panelEl.classList.add("at-bottom");
      }
      // Re-position note cards so they don't end up behind the panel
  ...
  function applyPanelCollapse() {
    ...
    if (panelCollapsed) {
      panelEl.classList.add("pi-panel-collapsed");
      collapseBtn.textContent = "▾";
      localStorage.setItem(PANEL_COLLAPSED_KEY, "1");
    } else {
      panelEl.classList.remove("pi-panel-collapsed");
      collapseBtn.textContent = "▴";
  ...
  function handleResize() {
    ...
    openNotes.forEach(index => {
      ...
      // Clamp based on panel position
      if (panel?.classList.contains("at-top")) {
        // Panel at top — cards shouldn't go above it
        if (rect.top < panelHeight + 16) {
          newY = panelHeight + 16;
          moved = true;
        }
      } else {
        // Panel at bottom — cards shouldn't go below it
  ...
  function repositionNotesForPanel() {
    ...
    openNotes.forEach(index => {
      ...
      if (panel.classList.contains("at-top")) {
        // Panel at top — make sure card isn't behind it
        if (rect.top < panelHeight + 16) {
          card.style.top = `${panelHeight + 16}px`;
          notePositions.set(index, { x: card.offsetLeft, y: panelHeight + 16 });
        }
      } else {
        // Panel at bottom — make sure card isn't behind it
        if (rect.bottom > vh - panelHeight - 16) {
          card.style.top = `${vh - rect.height - panelHeight - 16}px`;
          notePositions.set(index, { x: card.offsetLeft, y: vh - rect.height - panelHeight - 16 });
        }
      }
    });
    updateConnectors();
