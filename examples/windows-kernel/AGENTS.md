# AGENTS.md — Windows Kernel SPL Visualiser

This is the agent instructions file used to generate the `windows-kernel/` configurator in this repository.

The original source of this file is `~/VibeSettings/MicrosoftWindows/AGENTS.md` (the generic 7-view SPL template, used here for the Microsoft Windows kernel subsystem reference). The instructions below were used with the prompt template from the repository root README:

```
Follow AGENTS.md. Create a configurator for the Windows kernel. Here is more information: <Windows kernel subsystem / module reference information>.
```
