# AGENTS.md — Linux Kernel SPL Configurator

This is the agent instructions file used to generate the `linux-kernel/` configurator in this repository.
...
```
Follow AGENTS.md. Create a configurator for the Linux kernel. Here is more information: <linux-kernel/linux-2.6.33.3.uvl>.
```

The published artefact in this folder is the static `assets/` build output of the generated React + Vite project, plus the source `linux-2.6.33.3.uvl` and the derived `schema.uvl` / `schema.dot` files that were produced from it.
